#!/usr/bin/env python3
"""Generate an exact 5D B&B tree with refined seam-cache certificates."""
...
class RefinedChecker(ExactChecker):
    ...
    def containing_best(self, lo: F, hi: F):
        ...
        refinement_group = self.refinements_by_stable.get(stable_index)
        if refinement_group is None:
            return stable_index, stable_piece
        candidates = [
            (model_min(piece, lo, hi), index, piece)
            for index, piece in refinement_group
            if piece.lo <= lo and hi <= piece.hi
        ]
        if not candidates:
            return None
        candidates.append((model_min(stable_piece, lo, hi), stable_index, stable_piece))
        _value, index, piece = max(candidates, key=lambda row: (row[0], -row[1]))
        return index, piece

    def scalar_segments(self, lo: F, hi: F):
        assert lo <= hi
        boundaries = {lo, hi}
        for piece in self.catalog:
            if lo < piece.lo < hi:
                boundaries.add(piece.lo)
            if lo < piece.hi < hi:
                boundaries.add(piece.hi)
        ordered = sorted(boundaries)
        segments = []
        for left, right in zip(ordered, ordered[1:]):
            candidates = [
                (model_min(piece, left, right), index)
                for index, piece in enumerate(self.catalog)
                if piece.lo <= left and right <= piece.hi
            ]
            assert candidates
            value, index = max(candidates, key=lambda row: (row[0], -row[1]))
            segments.append(
                {"lo": qs(left), "hi": qs(right), "piece_index": index, "model_min": qs(value)}
            )
        if lo == hi:
            candidates = [
                (model_min(piece, lo, hi), index)
                for index, piece in enumerate(self.catalog)
                if piece.lo <= lo and hi <= piece.hi
            ]
            assert candidates
            value, index = max(candidates, key=lambda row: (row[0], -row[1]))
            segments = [{"lo": qs(lo), "hi": qs(hi), "piece_index": index, "model_min": qs(value)}]
        assert segments and q(segments[0]["lo"]) == lo and q(segments[-1]["hi"]) == hi
        assert all(q(a["hi"]) == q(b["lo"]) for a, b in zip(segments, segments[1:]))
        return segments
...
def main() -> None:
    ...
    manifest = {
        ...
        "trust_note": "Float proposals only; every terminal and refined seam-cache comparison uses exact Fraction arithmetic. Refinement soundness is a separate Lean WellCert replay.",
    }
