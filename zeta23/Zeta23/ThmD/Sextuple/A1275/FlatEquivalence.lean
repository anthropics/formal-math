...
/-!
# The two-level readers agree with the audited flat readers

`TreeWords.lean` stores the certificate words two levels deep; `Layout.lean` defines the
concatenated flat arrays `improvedTopologyWords` and `improvedLeafBlocks`. This module proves
that the streams actually replayed by the chunks are extensionally the audited flat streams:

* `improvedTopologyStream = packedTopologyStream improvedTokenCount improvedTopologyWords`,
* `improvedPayloadStream = improvedPackedLeafStream improvedLeafCount improvedLeafBlocks`.

The only fact about the concrete data used is that every block except the last has exactly
the declared block size (a linear Boolean check).
-/
...
/-- Linear Boolean form of `UniformBlocks`. -/
...
/-- Two-level reads agree with reads of the concatenation. -/
...
/-- `blockedWordRead` on arrays is `blockedListRead` on the underlying lists. -/
...
/-- The concrete topology blocks are uniform (`140` words each, the last at most that). -/
...
/-- The concrete leaf groups are uniform (`25` block words each, the last at most that). -/
...
/-- The replayed topology stream is the audited packed stream on the flat words. -/
...
/-- The replayed payload stream is the flat packed leaf stream on the flat blocks. -/
