...
| `a1285/` | the refined `A = 257/20000` certificate (`Zeta23/ThmD/Sextuple/A1285/`): canonical streams, exact replay report, generation report, frontier evidence for `A = 0.0128` / `0.0129` — see `a1285/README.md`; all Lean modules are generated by `tools/gen_sextuple_target_lean.py` |
| `a1290/` | the refined `A = 129/10000` certificate (`Zeta23/ThmD/Sextuple/A1290/`) over the 666-model catalog v3: canonical streams, exact replay, catalog v2/v3 generators with the exact `kernelRange` replica, the frontier analysis (method ceiling, `B`-scan), and the externally certified `A = 0.01293` frontier point — see `a1290/README.md` |
