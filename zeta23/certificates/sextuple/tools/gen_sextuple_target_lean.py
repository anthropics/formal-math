...
Outputs (under `--repo`, namespace tag `--ns`, e.g. `A1290`):
  ...
  Challenge/Sextuple<NS>.lean, Solution/Sextuple<NS>.lean, comparator-sextuple-<ns>.json, scripts/PrintAxioms/Sextuple<NS>.lean
...
Proof: Solution.{topic}.  Config: comparator-sextuple-{tag}.json.
...
'''
        ...
        pa = (f"import Solution.{topic}\nimport {self.mod}.LineDecimal\n\n"
              ...
              + "".join(f"#print axioms {n}\n" for n in names))
        return {f"Challenge/{topic}.lean": head + inlined + stmts,
                f"Solution/{topic}.lean": sol,
                f"comparator-sextuple-{tag}.json": cfg,
                f"scripts/PrintAxioms/{topic}.lean": pa}
...
def main() -> int:
    ...
    challenge_template = (a.repo / "Challenge/Sextuple.lean")
    if not challenge_template.exists() and a.validate_a1275 is not None:
        challenge_template = a.validate_a1275 / "Challenge/Sextuple.lean"
