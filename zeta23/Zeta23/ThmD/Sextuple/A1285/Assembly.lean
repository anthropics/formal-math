...
/-!
# Conditional sextuple assembly at `A = 257 / 20000`, `B = B₆`
...
is supplied by `Zeta23.ThmD.Sextuple.A1285.Certificate`.
-/
...
def zetaSextupleLedgerError : ℝ → ℝ :=
  zetaLedgerError (257 / 20000) (fun T => 10 * gramTransferError T)

theorem zetaSextupleLedgerInterface_of_certificate
    (hcertificate : ∀ g : Fin 5 → ℝ, (∀ i, 0 ≤ g i) →
      (257 / 20000 : ℝ) ≤ sextupleEnergy g + B6 * sextupleSpan g) :
    ZetaSextupleLedgerInterface
      (257 / 20000) B6 zetaSextuplePenalty zetaSextupleLedgerError := by
  change ZetaSextupleLedgerInterface
    (257 / 20000) B6 zetaSextuplePenalty
    (zetaLedgerError (257 / 20000) (fun T => 10 * gramTransferError T))
  exact zetaSextupleLedgerInterface_of_ordered_entry_close
    (A := (257 / 20000 : ℝ)) (B := B6) (entryError := gramTransferError)
...
theorem zetaSextupleLedgerError_isLittleO :
  ...
  change zetaLedgerError (257 / 20000) (fun T => 10 * gramTransferError T)
...
/-- The exact feedback constant at `A = 257 / 20000` and `B = B₆`. -/
def sextupleLowerConstant : ℝ :=
  feedbackConstant (HD 1) (257 / 20000) B6

theorem sextupleLowerConstant_exact :
    sextupleLowerConstant =
      (3000000000 * HD 1 - 1094977 * Real.pi) / 2993575000 := by
  rw [sextupleLowerConstant, feedbackConstant, B6]
  ring

/-- Strict fixed-rational comparison from the two directed analytic bounds. -/
theorem sextupleLowerConstant_gt_6727949489 :
    (6727949489 / 10 ^ 10 : ℝ) < sextupleLowerConstant := by
  rw [sextupleLowerConstant, feedbackConstant, B6]
  have hBMT := HD_one_decimal.1
  have hpi := Real.pi_lt_d20
  have hden : (0 : ℝ) < 6 - 257 / 20000 := by norm_num
  rw [lt_div_iff₀ hden]
...
theorem sextupleDyadic_of_interfaces
    ...
    (hledger : ZetaSextupleLedgerInterface
      (257 / 20000) B6 penalty ledgerError) :
    ∀ ε > 0, ∃ T₀ : ℝ, ∀ T ≥ T₀,
      (sextupleLowerConstant - ε) * (Ncount T (2 * T) : ℝ)
        ≤ N0simple T (2 * T) := by
  simpa only [sextupleLowerConstant] using
    (sextuple_zeta_dyadic_of_interfaces (A := (257 / 20000 : ℝ))
...
theorem sextupleCumulative_of_interfaces
    ...
    ∀ ε > 0, ∃ T₀ : ℝ, ∀ T ≥ T₀,
      (sextupleLowerConstant - ε) * (Ncount 0 T : ℝ)
        ≤ N0simple 0 T := by
  simpa only [sextupleLowerConstant] using
    (sextuple_zeta_cumulative_of_interfaces (A := (257 / 20000 : ℝ))
      (B := B6) (by norm_num) hbase hledger)
...
theorem sextupleDyadic_6727949489_of_interfaces
    {penalty baseError ledgerError : ℝ → ℝ}
    (hbase : ZetaBasePenaltyInterface penalty baseError)
    (hledger : ZetaSextupleLedgerInterface
      (257 / 20000) B6 penalty ledgerError) :
    ∃ T₀ : ℝ, ∀ T ≥ T₀,
      (6727949489 / 10 ^ 10 : ℝ) * (Ncount T (2 * T) : ℝ)
        ≤ N0simple T (2 * T) :=
  fixedLowerOfEpsForm sextupleLowerConstant_gt_6727949489
    (sextupleDyadic_of_interfaces hbase hledger)

theorem sextupleCumulative_6727949489_of_interfaces
    {penalty baseError ledgerError : ℝ → ℝ}
    (hbase : ZetaBasePenaltyInterface penalty baseError)
    (hledger : ZetaSextupleLedgerInterface
      (257 / 20000) B6 penalty ledgerError) :
