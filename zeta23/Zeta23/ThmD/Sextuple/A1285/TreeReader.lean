import Zeta23.ThmD.Sextuple.A1285.ScalarData
import Zeta23.ThmD.Sextuple.Macro.ParametricAdapter

/-!
# Readers and the concrete leaf checker for the `A = 257 / 20000` certificate
...
abbrev improvedA : ℚ := 257 / 20000
abbrev improvedB : ℚ := 1094977 / 5000000000
abbrev improvedCutoff : ℚ := 64250000 / 1094977
...
def improvedDecodeTermRef (code : ℕ) : Option (MacroTermRef 272 3365) :=
  if h : code < 272 then some (.piece ⟨code, h⟩)
  else if code = 65535 then some .zero
  else if h : 32768 ≤ code ∧ code - 32768 < 3365 then
    some (.scalar ⟨code - 32768, h.2⟩)
  else none

def improvedLeafTerm (w : ℕ) (p : Fin 15) : MacroTermRef 272 3365 :=
...
def improvedDecodeLeafWord (w : ℕ) :
    Option (AffineLeafPayload (MacroScalarLeaf 272 3365)) :=
...
def improvedPackedLeafStream (leafCount : ℕ) (blocks : Array ℕ) :
    CursorStream (AffineLeafPayload (MacroScalarLeaf 272 3365)) where
  length := leafCount
  read := fun p =>
...
def improvedConcreteLeafCheck :
    GapBox → AffineLeafPayload (MacroScalarLeaf 272 3365) → Bool :=
